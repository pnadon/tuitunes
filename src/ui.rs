...
use tui::{
  ...
  widgets::{BarChart, Block, Borders, List, ListItem, self, Clear, Paragraph},
  Frame,
};

use crate::spectrum::Analyzer;

pub fn main_ui<B: Backend>(f: &mut Frame<B>, analyzer: &Analyzer, song_name: &str, up_next: &[&str], history: &[&str], ui_color: Color) {
  let data = analyzer
    .data()
    .iter()
